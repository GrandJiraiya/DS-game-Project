...
if [ "$WF_USE_MUSL" == "false" ]; then
	remove_dependencies runtime-gcc-libs
	remove_dependencies runtime-musl-dev runtime-musl
	remove_dependencies runtime-zlib-dev runtime-zlib
	remove_dependencies runtime-zstd-dev runtime-zstd
	if [ "x$?" == "x0" ]; then
